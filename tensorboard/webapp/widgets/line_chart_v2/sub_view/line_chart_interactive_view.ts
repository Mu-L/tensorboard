...
export class LineChartInteractiveViewComponent
  implements XDimChartView, YDimChartView, OnChanges, OnDestroy {
    ...
    fromEvent<MouseEvent>(this.dotsContainer.nativeElement, 'mouseup', {
      ...
      .subscribe(() => {
        ...
        if (
          ...
          const xMin = getDataX(this, zoomBox.x);
          const xMax = getDataX(this, zoomBox.x + zoomBox.width);
          const yMin = getDataY(this, zoomBox.y + zoomBox.height);
          const yMax = getDataY(this, zoomBox.y);
      ...
      .subscribe((event) => {
        switch (this.state) {
          ...
          case InteractionState.PANNING: {
            ...
            const xMin = getDataX(this, deltaX);
            const xMax = getDataX(this, domWidth + deltaX);
            const yMin = getDataY(this, domHeight + deltaY);
            const yMax = getDataY(this, deltaY);
            this.onViewExtentChange.emit({
              x: [xMin, xMax],
              y: [yMin, yMax],
            });
  ...
  getDomX(uiCoord: number): number {
    return getDomX(this, uiCoord);
  }

  getDomY(uiCoord: number): number {
    return getDomY(this, uiCoord);
  }

  private updateTooltip(event: MouseEvent) {
    this.cursorXLocation = getDataX(this, event.offsetX);
    this.updateCursoredDataAndTooltipVisibility();
  }
